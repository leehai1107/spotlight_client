describe("Bar chart tests", function () {
  ...
  describe("Meta data tests", function () {
    it("should render meta data correctly with mixed value array", function (done) {
      ...
      chart.on("created", function () {
        expect(
          Chartist.deserialize($(".ct-bar").eq(3).attr("ct:meta"))
        ).toEqual(meta);
        done();
      });
    });
    ...
    it("should render meta data correctly with mixed value array and mixed series notation", function (done) {
      ...
      chart.on("created", function () {
        expect(
          Chartist.deserialize($(".ct-series-a .ct-bar").eq(3).attr("ct:meta"))
        ).toEqual(valueMeta);
        expect(Chartist.deserialize($(".ct-series-b")).attr("ct:meta")).toEqual(
          "" + seriesMeta
        );
        expect(
          Chartist.deserialize($(".ct-series-b .ct-bar").eq(2).attr("ct:meta"))
        ).toEqual(valueMeta);
        done();
      });
    });
