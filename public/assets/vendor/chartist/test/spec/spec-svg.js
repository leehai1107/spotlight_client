...
describe("Chartist SVG", function () {
  ...
  it("should create nested objects with attributes", function () {
    ...
    expect(
      svg._node.firstChild.firstChild.firstChild.attributes.cx.textContent
    ).toBe("100");
    expect(
      svg._node.firstChild.firstChild.firstChild.attributes.cy.textContent
    ).toBe("100");
    expect(
      svg._node.firstChild.firstChild.firstChild.attributes.r.textContent
    ).toBe("10");
  });
  ...
  it("should allow to set namespaced attributes", function () {
    ...
    expect(
      svg._node.firstChild.getAttributeNS(Chartist.namespaces.xlink, "href")
    ).toBe("image.jpg");
  });
  ...
  it("should allow to travers up in the fluent API chain and set attributes on the way", function () {
    ...
    expect(svg._node.firstChild).toBeDefined();
    expect(svg._node.firstChild.attributes.transform.textContent).toBe(
      "rotate(30 30 30)"
    );

    expect(svg._node.firstChild.firstChild).toBeDefined();
    expect(
      svg._node.firstChild.firstChild.attributes.transform.textContent
  ...
  describe("path tests", function () {
    ...
    it("should parse MDN SVG path style correctly", function () {
      var path = new Chartist.Svg.Path().parse(
    ...
    it("should split correctly by move command", function () {
      var paths = new Chartist.Svg.Path()
