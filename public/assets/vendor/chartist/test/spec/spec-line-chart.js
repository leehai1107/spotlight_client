describe("Line chart tests", function () {
  ...
  describe("grids", function () {
    ...
    function onCreated(fn) {
      jasmine
        .getFixtures()
        .set('<div class="ct-chart ct-golden-section"></div>');
  ...
  describe("Meta data tests", function () {
    it("should render meta data correctly with mixed value array", function (done) {
      ...
      chart.on("created", function () {
        expect(
          Chartist.deserialize($(".ct-point").eq(3).attr("ct:meta"))
        ).toEqual(meta);
        done();
      });
    });
    ...
    it("should render meta data correctly with mixed value array and mixed series notation", function (done) {
      ...
      chart.on("created", function () {
        expect(
          Chartist.deserialize(
            $(".ct-series-a .ct-point").eq(3).attr("ct:meta")
          )
        ).toEqual(valueMeta);
        expect(Chartist.deserialize($(".ct-series-b")).attr("ct:meta")).toEqual(
          "" + seriesMeta
        );
        expect(
          Chartist.deserialize(
            $(".ct-series-b .ct-point").eq(2).attr("ct:meta")
          )
        ).toEqual(valueMeta);
        done();
      });
    ...
    it("should render correctly with Interpolation.none and holes everywhere", function (done) {
      ...
      chart.on("draw", function (context) {
        if (context.type === "line") {
          expect(
            context.path.pathElements.map(function (pathElement) {
              return {
