...
describe("mixitup()", () => {
  it("should throw an error if no container reference", () => {
    chai.assert.throws(
      () => mixitup(),
      Error,
      mixitup.messages.errorFactoryInvalidContainer()
    );
  });
  ...
  it("should accept a container selector", () => {
    ...
    chai.assert.equal(
      state.container,
      frag.querySelector(".mixitup-container")
    );
  ...
  it("should throw an error if the container selector yields no element", () => {
    chai.assert.throws(
      ...
      Error,
      mixitup.messages.errorFactoryContainerNotFound()
    );
  });
  ...
  it("should return a single instance of a mixer, wrapping the first element if multiple elements passed", () => {
    let elementList = [dom.getContainer(), dom.getContainer()];
  ...
  it("should respect a `load.sort` configuration option", () => {
    let idsByPublishedDate = dataset
      .slice()
      .sort((a, b) => {
