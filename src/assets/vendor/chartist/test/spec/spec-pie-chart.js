  ...
  describe("Meta data tests", function () {
    it("should render meta data correctly on slice with mixed value array", function (done) {
      ...
      chart.on("created", function () {
        expect(
          Chartist.deserialize($(".ct-slice-pie").eq(1).attr("ct:meta"))
        ).toEqual(meta);
        done();
      });
    });
  ...
  describe("Pie with empty values", function () {
    ...
    it("Pie should render without NaN values and points", function (done) {
      onCreated(function () {
        ...
        expect(slices.eq(0).attr("d")).toBe(
          "M200,5A118.609,118.609,0,0,0,200,5L200,123.609Z"
        );
        expect(slices.eq(1).attr("d")).toBe(
          "M200,5A118.609,118.609,0,0,0,200,5L200,123.609Z"
        );
        expect(slices.eq(2).attr("d")).toBe(
          "M200,5A118.609,118.609,0,0,0,200,5L200,123.609Z"
        );
        done();
      });
