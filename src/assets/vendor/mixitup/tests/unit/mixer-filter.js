...
describe("mixitup.Mixer", () => {
  describe("#filter()", () => {
    ...
    it("should accept a class selector", () => {
      let matching = Array.prototype.slice.call(
        container.querySelectorAll(".category-a")
      );

      return mixer.filter(".category-a").then((state) => {
    ...
    it("should accept an attribute selector", () => {
      let matching = Array.prototype.slice.call(
        container.querySelectorAll('[data-category~="a"]')
      );

      return mixer.filter('[data-category~="a"]').then((state) => {
        chai.assert.equal(state.totalShow, matching.length);
        chai.assert.deepEqual(state.show, matching);
        chai.assert.deepEqual(state.matching, matching);
      });
    ...
    it("should accept a compound OR class selector", () => {
      let matching = Array.prototype.slice.call(
        container.querySelectorAll(".category-a, .category-b")
      );

      return mixer.filter(".category-a, .category-b").then((state) => {
        chai.assert.equal(state.totalShow, matching.length);
        chai.assert.deepEqual(state.show, matching);
        chai.assert.deepEqual(state.matching, matching);
      });
    ...
    it("should accept a compound AND class selector", () => {
      let matching = Array.prototype.slice.call(
        container.querySelectorAll(".category-a.category-c")
      );

      return mixer.filter(".category-a.category-c").then((state) => {
        chai.assert.equal(state.totalShow, matching.length);
        chai.assert.deepEqual(state.show, matching);
        chai.assert.deepEqual(state.matching, matching);
      });
    ...
    it("should accept a compound OR attribute selector", () => {
      let matching = Array.prototype.slice.call(
        container.querySelectorAll('[data-category~="a"], [data-category~="c"]')
      );

      return mixer
        .filter('[data-category~="a"], [data-category~="c"]')
        .then((state) => {
          chai.assert.equal(state.totalShow, matching.length);
          chai.assert.deepEqual(state.show, matching);
          chai.assert.deepEqual(state.matching, matching);
    ...
    it('should accept "none"', () => {
      return mixer.filter("none").then((state) => {
    ...
    it('should accept "all"', () => {
      return mixer.filter("all").then((state) => {
        chai.assert.deepEqual(
          state.show,
          Array.prototype.slice.apply(container.children)
        );
    ...
    it("should fail if queried with a non matching selector", () => {
      return mixer.filter(".non-mathing-selector").then((state) => {
    ...
    it("should accept a single element", () => {
      ...
      return mixer.filter(el).then((state) => {
    ...
    it("should accept a collection of elements", () => {
      ...
      return mixer.filter(collection).then((state) => {
    ...
    it("should interpret `null` as hide all", () => {
      return mixer.filter(null).then((state) => {
        chai.assert.deepEqual(state.show, []);
        chai.assert.equal(state.activeFilter.selector, "");
    ...
    it("should accept a full CommandFilter object, allowing for inverse filtering via selector", () => {
      ...
      let collection = Array.prototype.slice.call(
        container.querySelectorAll(":not(.category-a)")
      );

      return mixer.filter(command).then((state) => {
        chai.assert.deepEqual(state.show, collection);
    ...
    it("should accept a full CommandFilter object, allowing for inverse filtering via a collection", () => {
      ...
      let collection = Array.prototype.slice.call(
        container.querySelectorAll(":not(.category-a.category-c)")
      );
    ...
    it("should accept a boolean allowing toggling off of animation", () => {
      ...
      return mixer.filter(".category-a", false).then((state) => {
        chai.assert.equal(state.totalShow, matching.length);
        chai.assert.deepEqual(state.show, matching);
        chai.assert.deepEqual(state.matching, matching);
      });
    ...
    it("should throw an error if both a selector and a collection are provided", () => {
      ...
      chai.assert.throws(
        ...
        Error,
        mixitup.messages.errorFilterInvalidArguments()
      );
    });
  });
});
