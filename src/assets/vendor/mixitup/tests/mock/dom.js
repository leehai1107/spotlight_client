...
module.exports = {
  getContainer() {
    return renderElement(
      '<div class="mixitup-container" data-ref="container">' +
  ...
  getTarget() {
    return renderElement(
      '<div id="7" class="mix category-d" data-ref="mix" data-category="d" data-published="20161222" data-views="132"></div>'
    );
  },
  ...
  getSortControl() {
    return renderElement(
      '<div class="mixitup-control" data-sort="views:desc published:asc">Views (desc) Published (asc)</div>'
    );
  },
  ...
  Item: class Item {
    ...
    get classList() {
      return (
        "mix " +
        this.categories.map((category) => "category-" + category).join(" ")
      );
    }
  ...
  ITEM_TEMPLATE:
    '<div id="${id}" class="${classList}" data-ref="mix" data-category="${categoryList}" data-published="${published}" data-views="${views}"></div>',
